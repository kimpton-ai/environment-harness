...
def setup(path, limit=4096):
    ...
    spec = ExperimentSpec(
        ...
        participants=tuple(
            AgentSpec(id=p, implementation=SyntheticAgent.implementation, policy_version="1")
            for p in ("a", "b")
        ),
        scoring_versions=("accuracy@1", "accuracy@2", "cost@1"),
        policy=RunPolicy(max_event_bytes=limit),
    )
...
def report(
    ...
    *,
    ...
    definitions = (
        {} if unit is None else {metric: {"id": metric, "version": definition_version, "unit": unit}}
    )
    return store.report(
        environment,
        who,
...
@pytest.mark.parametrize(
    "difference", ["unit", "scorer_version", "definition_version", "kind", "participant", "split", "policy"]
)
def test_incompatible_scores_never_share_statistics(tmp_path, difference):
    ...
    if difference == "participant":
        other_spec = spec.model_copy(
            update={
                "participants": tuple(p.model_copy(update={"policy_version": "2"}) for p in spec.participants)
            }
        )
    elif difference == "split":
        other_spec = spec.model_copy(
            update={"scenario": "training", "split": "training", "purpose": "training"}
        )
...
def test_large_history_is_lossless_private_and_does_not_grow_on_rebranch(tmp_path, text):
    ...
    with store.transaction() as db:
        ...
        original_row = dict(
            db.execute(
                "SELECT * FROM events WHERE environment=? AND seq=?", (parent, original["seq"])
            ).fetchone()
        )
    lease = session.lease(parent, who, "checkpoint")
    ...
    assert not any(
        r["record"]["kind"] == "private.large" for r in reconstruct_inherited(store.replay(child, bob))
    )
    ...
    for _ in range(3):
        ...
        assert (
            next(
                r["record"] for r in reconstruct_inherited(history) if r["record"]["kind"] == "private.large"
            )
            == original_row
        )
...
def test_legacy_integral_timestamp_and_reports_remain_readable(tmp_path):
    ...
    with store.transaction() as db:
        last = db.execute(
            "SELECT hash FROM events WHERE environment=? ORDER BY seq DESC", (environment,)
        ).fetchone()[0]
        ...
        db.execute(
            ...
            (
                ...
                1,
                encode(body),
                digest({"environment": environment, "revision": 1, "report": body}),
            ),
        )
