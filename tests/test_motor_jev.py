...
def test_budget_cancellation_deadline_and_size_fail_closed_without_transport():
    ...
    assert (
        selector.select(
            {}, candidates(), maximum_cost_micros=100, cancel=event, deadline=time.monotonic() + 1
        ).candidate_id
        is None
    )
    with pytest.raises(JevBudgetExceeded):
        selector.select(
            {}, candidates(), maximum_cost_micros=0, cancel=threading.Event(), deadline=time.monotonic() + 1
        )
    assert not calls
...
def test_maximum_cost_is_conservative_and_duplicate_candidates_are_rejected():
    ...
    assert (
        selector.select(
            {}, duplicate, maximum_cost_micros=100, cancel=threading.Event(), deadline=time.monotonic() + 1
        ).candidate_id
        is None
    )
...
def test_reserved_abstain_is_valid_and_candidate_collision_is_rejected():
    ...
    assert (
        selector.select(
            {}, colliding, maximum_cost_micros=100, cancel=threading.Event(), deadline=time.monotonic() + 1
        ).candidate_id
        is None
    )
...
def test_model_mismatch_and_usage_over_reservation_are_unknown():
    ...
    over = JevSelector(
        "secret", transport=lambda *_: {**response(), "usage": {"input_tokens": 10_000, "output_tokens": 10}}
    )
    with pytest.raises(JevBudgetExceeded):
        over.select(
            {},
...
def test_cancellation_after_dispatch_preserves_valid_usage_cost():
    ...
    selector = JevSelector("secret", transport=transport)
    result = selector.select(
        {}, candidates(), maximum_cost_micros=100, cancel=cancel, deadline=time.monotonic() + 1
    )
    assert result.candidate_id is None
