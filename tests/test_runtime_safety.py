...
def setup(path, *, participants=('a',), policy=None, checkpoint=True):
    ...
    spec = ExperimentSpec(environment=env.spec, participants=tuple(
        AgentSpec(id=p, implementation=SyntheticAgent.implementation, policy_version='1', checkpoint=checkpoint)
...
def test_old_worker_cannot_replace_recovery_or_continuation(tmp_path, replacement, fail):
    ...
    with ThreadPoolExecutor() as pool:
        ...
        try:
            ...
            if replacement == 'reconcile':
                session.reconcile_agent(environment, who, new, operation_id=work['id'], response={'value': -1},
                                        agent_state={'recovered': True}, evidence={'receipt': 'synthetic'})
    ...
    with session.store.transaction() as db:
        ...
        state = json.loads(db.execute('SELECT participants FROM environments WHERE id=?', (environment,)).fetchone()[0])
    if replacement == 'reconcile':
...
def test_conformance_modes_and_lease_cleanup(tmp_path, mode, deadline, checkpoint):
    ...
    env.spec = env.spec.model_copy(update={'phase_deadline': deadline,
        'capabilities': Capabilities(checkpoint=checkpoint)})
...
def test_cancel_is_atomic_idempotent_and_preserves_unknown_reservations(tmp_path):
    ...
    for key in ('prepared', 'unknown'):
        operations.prepare(environment, agent, key, endpoint='synthetic', operation='test', payload={}, maximum_cost_micros=10)
    with session.store.transaction() as db:
...
def test_cancel_active_command_stops_parent_and_child(tmp_path, transport):
    ...
    program = CommandAgent([sys.executable, '-c', code, str(marker), child_code], SyntheticAgent.implementation)
    with ThreadPoolExecutor() as pool:
        ...
        try:
            ...
            if transport == 'cli':
                result = subprocess.run([sys.executable, '-m', 'environment_harness.cli', '--store', str(tmp_path / 'store'),
                                         'cancel', environment], capture_output=True, text=True, timeout=5)
                assert result.returncode == 0, result.stderr
                result = json.loads(result.stdout)
            else:
                token = session.store.issue(who)
                client = TestClient(create_app(session))
                response = client.post(f'/v1/environments/{environment}/commands',
                    json={'operation': 'cancel', 'arguments': {}}, headers={'Authorization': 'Bearer ' + token})
...
def test_cli_releases_checkpoint_and_resume_leases(tmp_path):
    ...
    for command in ('checkpoint', 'resume'):
        result = subprocess.run([sys.executable, '-m', 'environment_harness.cli', '--store', str(tmp_path),
                                 command, environment], capture_output=True, text=True)
        assert result.returncode == 0, result.stderr
