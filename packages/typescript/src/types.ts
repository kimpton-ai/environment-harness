...
export interface EnvironmentSpec {protocol: 'environment-session.v1'; id: string; version: string; implementation: string; observation_schema: Record<string,Json>; action_schema: Record<string,Json>; scenario_schema: Record<string,Json>; scheduling: 'sequential'|'simultaneous'|'event'; modalities: string[]; capabilities: Capabilities; purposes: ('evaluation'|'training')[]; stale_action: 'reject'; missing_action: 'reject'|'noop'; phase_seconds: number; phase_deadline: 'wall'|'coordinator'; motor_skills?: string[];}
...
export interface ExperimentSpec {environment:EnvironmentSpec; participants:AgentSpec[]; seed:number; scenario:string; scenario_input:Json; scenario_reference:Json; scenario_metadata:Record<string,Json>; split:'training'|'heldout'; purpose:'evaluation'|'training'; time_boundary:string; interventions:Record<string,Json>; scoring_versions:string[]; policy:RunPolicy; motor?:MotorProfile|null;}
export interface MotorProfile {mode?: 'deterministic'|'jev'; executor?: 'motor.v1'; adapter:string; selector_model?: string|null;}
export interface MilestoneIdentity {goal_id:string; milestone_id:string; revision:string;}
export interface GoalContext {goal_id:string; revision:string; milestone_id:string;}
export interface ResourceOwnership {resource:string; owner:string; namespace:string;}
export interface ControlClaim {channel:string; owner:string; owner_namespace:string; controls:Record<string,Json>; resources?:ResourceOwnership[];}
export interface ProgressReceipt {group_id:string; tick:number; channel:string; status:'completed'|'blocked'|'cancelled'; operation_id:string; stop_epoch:number;}
export interface MotorControlPermission {id:string; controls?:Record<string,Json>; max_steps?:number; max_travel?:number|null; protected_region_revision?:string|null;}
export interface MotorRequest {skill:string; target:Record<string,Json>; arguments?:Record<string,Json>; expected:Record<string,Json>; observation_revision:string; goal_revision:string; stop_epoch?:number; max_steps?:number; timeout_ms?:number; control_permissions?:MotorControlPermission[]; max_recovery_attempts?:number; goal_context?:GoalContext|null; group?:MotorGroup|null;}
export interface MotorStep {operation:string; target:Record<string,Json>; arguments?:Record<string,Json>; controls?:Record<string,Json>;}
...
export interface MotorReceipt {operation_id:string; status:'completed'|'blocked'|'cancelled'; outcome?:'rejected'|'accepted'|'started'|'completed'|'cancelled'|'unknown'; effect?:'none'|'possible'|'applied'; cost_micros?:number; profile:MotorProfile; request:MotorRequest; reason?:string|null; reason_code?:string|null; before?:Record<string,Json>; after?:Record<string,Json>; selection?:MotorSelection|null; steps?:Record<string,Json>[]; elapsed_ms:number;}
export interface MotorGroup {group_id:string; goal_context:GoalContext; claims:ControlClaim[]; max_ticks?:number; deadline_ms?:number; stop_epoch?:number;}
export interface MotorGroupReceipt {group_id:string; status:'completed'|'blocked'|'cancelled'; goal_context:GoalContext; stop_epoch:number; progress?:ProgressReceipt[]; operation_id:string; cost_micros?:number; elapsed_ms:number;}
export interface MotorExecutionMetadata {contract_version:'motor.execution.v1'; owner:string; owner_id?:string; epoch:number; goal_revision:string; observation_revision:string; stop_epoch:number; interface_revision?:string|null; ui_revision?:string|null; observation_frame_id?:string|null; camera_revision?:string|null; observed_at_ms?:number|null; native_tick?:number|null;}
export interface PreparedSuccessorIntent {contract_version:'motor.prepared-successor.v1'; intent_id:string; operation_id:string; predecessor_operation_id:string; candidate_id:string; metadata:MotorExecutionMetadata; step:MotorStep; prepared_at_ms:number; freshness_ms:number; expires_tick?:number|null; phase:'prepared';}
export interface PreparedSuccessorAdmission {contract_version:'motor.prepared-successor-admission.v1'; intent_id:string; predecessor_operation_id:string; operation_id:string; metadata:MotorExecutionMetadata; status:'admitted'|'rejected'|'unknown'; reason_code?:string|null; admitted_at_ms?:number|null;}
...
export interface MotorExecutor {acknowledge_unknown():{operation_ids:string[]; observation:Record<string,Json>; stop_epoch:number}; progress(operation_id:string):{status:string; steps:Record<string,Json>[]} | null; execute(operation_id:string, request:Record<string,Json>, maximum_cost_micros:number):MotorReceipt; prepare_successor(intent:PreparedSuccessorIntent, request:MotorRequest, selection:MotorSelection, authority:(request:MotorRequest)=>Record<string,Json>):PreparedSuccessorIntent; admit_successor(intent_id:string, request:MotorRequest, selection:MotorSelection, authority:(request:MotorRequest)=>Record<string,Json>, predecessor?:MotorReceipt):PreparedSuccessorAdmission; reconcile_successor(intent_id:string):PreparedSuccessorAdmission; reconcile(operation_id:string):Record<string,Json>|null; stop():void; lookup(operation_id:string):MotorReceipt|null;}
...
export interface ActivityExperiment {kind:'experiment'; id:string; name:string; status:string; progress:{completed:number;total:number}; running:number; queued:number; failed:number; latest_activity:string|null; score_summary:Record<string,number>; scenarios:ActivityScenario[]; sessions:ActivitySession[]; updated:number;}
