...
import {
  MISSING, SLOTS, buildTimeline, buildTurnSeries, cellText, describe, filterEvents, formatCompactCount, formatCost, formatTime, hasActivity,
  inheritedSentence, shortId, summarizeReports, title, turnLabel, scalars, type Slot, type Turn,
} from './timeline.js';
import type {ActivityEvent, ActivityExperiment, ActivityScenario, ActivitySession, ActivitySnapshot, EvidenceEvent, Json, Environment, Comparison, ReportEnvelope, TurnSeriesResponse} from './types.js';
...
// Helpers
function message(value: string, kind: 'status' | 'error' = 'status') {
  if (!value) return;
  const region = el('toast-region');
  const toast = text('div', '', `toast toast-${kind}`);
  toast.setAttribute('role', kind === 'error' ? 'alert' : 'status');
  toast.setAttribute('aria-atomic', 'true');
  const dismiss = text('button', '×', 'toast-dismiss') as HTMLButtonElement;
  dismiss.type = 'button'; dismiss.setAttribute('aria-label', 'Dismiss notification'); dismiss.title = 'Dismiss notification';
  let timer = 0;
...
async function attempt(fn: () => Promise<void>) {try {await fn();} catch (error) {message(error instanceof Error ? error.message : 'Operation failed', 'error');}}
function text(tag: string, value: string, className = '') {const node = document.createElement(tag); node.textContent = value; if (className) node.className = className; return node;}
function details(summary: string, body: HTMLElement, className = '', id = '') {
  const node = document.createElement('details'); node.className = className; if (id) node.dataset.eventId = id;
  node.append(text('summary', summary), body); return node;
}
function bindTurnRangeTrack(
  track: HTMLElement, selection: HTMLElement, start: HTMLInputElement, end: HTMLInputElement,
  onPreview: () => void, onCommit?: () => void,
) {
  ...
  selection.onpointerdown = event => {
    ...
    let changed = false;
    event.preventDefault(); selection.classList.add('is-dragging'); selection.setPointerCapture(event.pointerId);
    ...
    selection.onpointermove = move => {
      ...
      start.value = String(nextFirst); end.value = String(nextFirst + span); onPreview();
    };
    const finish = (release: PointerEvent) => {
      if (selection.hasPointerCapture(release.pointerId)) selection.releasePointerCapture(release.pointerId);
      selection.classList.remove('is-dragging');
      selection.onpointermove = null; selection.onpointerup = null; selection.onpointercancel = null;
      if (changed) onCommit?.();
    };
    selection.onpointerup = finish; selection.onpointercancel = finish;
  };
  track.onwheel = event => {
    const first = Number(selection.dataset.first), last = Number(selection.dataset.last);
    const maximum = Number(selection.dataset.maximum), span = last - first;
    if (maximum <= 1 || span >= maximum - 1) return;
    const dominantDelta = Math.abs(event.deltaX) > Math.abs(event.deltaY) ? event.deltaX : event.deltaY;
    if (!dominantDelta) return;
    event.preventDefault(); selection.classList.add('is-scrolling');
    ...
    wheelTimer = window.setTimeout(() => {
      wheelDelta = 0; selection.classList.remove('is-scrolling');
      if (wheelChanged) {wheelChanged = false; onCommit?.();}
    }, 160);
...
function dropdownLabel(select: HTMLSelectElement) {
...
function enhanceDropdown(select: HTMLSelectElement) {
  const existing = select.closest<HTMLElement>('.viewer-dropdown');
  if (existing) return existing;
  const shell = text('div', '', 'viewer-dropdown');
  select.before(shell); shell.append(select);
  const menuId = `${select.id}-options`;
  const trigger = text('button', '', 'dropdown-trigger') as HTMLButtonElement;
  trigger.id = `${select.id}-trigger`; trigger.type = 'button';
  trigger.setAttribute('aria-haspopup', 'listbox'); trigger.setAttribute('aria-expanded', 'false'); trigger.setAttribute('aria-controls', menuId);
  trigger.append(text('span', ''));
  const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  svg.setAttribute('viewBox', '0 0 12 12'); svg.setAttribute('aria-hidden', 'true');
  const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
  path.setAttribute('d', 'm3 4.5 3 3 3-3'); path.setAttribute('fill', 'none'); path.setAttribute('stroke', 'currentColor');
  path.setAttribute('stroke-linecap', 'round'); path.setAttribute('stroke-linejoin', 'round'); svg.append(path); trigger.append(svg);
  const menu = text('div', '', 'dropdown-menu'); menu.id = menuId; menu.role = 'listbox'; menu.setAttribute('aria-label', dropdownLabel(select)); menu.hidden = true;
  shell.append(trigger, menu);
  return shell;
}
function syncDropdown(select: HTMLSelectElement) {
  const shell = select.closest<HTMLElement>('.viewer-dropdown');
  const trigger = shell?.querySelector<HTMLButtonElement>('.dropdown-trigger');
  ...
  menu.replaceChildren(...[...select.options].map(option => {
    const item = text('button', option.textContent, 'dropdown-option') as HTMLButtonElement;
    item.type = 'button'; item.role = 'option'; item.dataset.value = option.value;
    item.setAttribute('aria-selected', String(option.selected)); item.tabIndex = option.selected ? 0 : -1;
    item.onclick = () => {
    ...
    item.onkeydown = event => {
      const options = [...menu.querySelectorAll<HTMLButtonElement>('.dropdown-option')];
      ...
      if (event.key === 'Escape') {
        event.preventDefault(); menu.hidden = true; trigger.setAttribute('aria-expanded', 'false'); trigger.focus();
      }
    };
    return item;
  }));
...
function initializeDropdown(select: HTMLSelectElement) {
  ...
  select.dataset.dropdownInitialized = 'true'; select.classList.add('dropdown-native'); select.setAttribute('aria-hidden', 'true'); select.tabIndex = -1;
  const close = () => {menu.hidden = true; trigger.setAttribute('aria-expanded', 'false');};
  const open = () => {
    syncDropdown(select); menu.hidden = false; trigger.setAttribute('aria-expanded', 'true');
    requestAnimationFrame(() => {if (!menu.hidden) menu.querySelector<HTMLButtonElement>('[aria-selected="true"]')?.focus();});
  };
  ...
  document.addEventListener('pointerdown', event => {if (!shell.contains(event.target as Node)) close();});
  syncDropdown(select);
}
const participantName = (name: string) =>
  name.replaceAll('_', ' ').replace(/^./, letter => letter.toUpperCase());
const sessionStatus = (status: string) => status === 'succeeded' ? 'Completed' : participantName(status);
const statusFilter = (status: string) => ['succeeded', 'completed'].includes(status) ? 'completed' : status;
function scenarioTitle(item: {id: string; metadata: Record<string, Json>}) {
  const name = item.metadata.name;
  return typeof name === 'string' && name.trim() ? name : item.id;
}
function groupedSessionTitle(id: string | undefined) {
  if (!id || !activitySnapshot) return null;
  for (const experiment of activitySnapshot.experiments) {
    for (const scenario of experiment.scenarios) {
      ...
      if (session) return `${scenarioTitle(scenario)} · Trial ${session.trial}`;
    }
  }
  return null;
}
function sessionTitle(item: {participants?: string[]; id?: string; parent?: string | null; experiment?: Record<string, Json>}) {
  ...
  if (names.length > 3) return `${names.length} Participants`;
  if (names.length) {
...
function mapping(value: Json | undefined): Record<string, Json> {
...
function positiveTurn(value: string | null) {
  const parsed = Number(value);
  return Number.isInteger(parsed) && parsed >= 1 ? parsed : undefined;
}
function selectionPath(path: '/home' | '/compare', environments: string[], view: ComparisonViewState = {}) {
  const parameters = new URLSearchParams();
  for (const environment of [...new Set(environments)].slice(0, 100)) parameters.append('environment', environment);
  if (path === '/compare' && view.metric) parameters.set('metric', view.metric);
  if (path === '/compare' && view.startTurn !== undefined) parameters.set('start_turn', String(view.startTurn));
  if (path === '/compare' && view.endTurn !== undefined) parameters.set('end_turn', String(view.endTurn));
  const query = parameters.toString();
  return path + (query ? `?${query}` : '');
}
function routeFromLocation(): ViewerRoute | null {
  const path = location.pathname.replace(/\/$/, '') || '/';
  const parameters = new URLSearchParams(location.search);
  const environments = selectedEnvironments(parameters);
  if (path === '/' || path === '/home') return {kind: 'home', environments};
  if (path === '/compare') {
    ...
    return {kind: 'compare', environments, view: {
      metric: comparisonMetricDefinitions.some(candidate => candidate.id === metric) ? metric as ComparisonMetricId : undefined,
      startTurn,
...
function setLocation(path: string, replace = false) {
...
function restoreSelected(environments: string[]) {
  selected.clear();
  for (const id of environments) if (known(id)) selected.add(id);
}
function persistSelection() {
  const route = routeFromLocation();
  if (route?.kind === 'home') setLocation(selectionPath('/home', [...selected]), true);
  if (route?.kind === 'compare') setLocation(selectionPath('/compare', [...selected], route.view), true);
}
function markListSynced() {
  const now = new Date();
  const node = el<HTMLTimeElement>('list-synced');
...
async function list() {
  ...
  activitySnapshot = snapshot; activityCursor = Math.max(activityCursor, snapshot.cursor);
  renderList();
  renderModeSummary();
  markListSynced();
  return catalog;
...
async function refreshFromButton() {
  ...
  try {
    ...
    button.classList.remove('is-loading'); button.removeAttribute('aria-busy'); button.removeAttribute('aria-label');
...
async function refreshCurrentSession() {
  ...
  const turnScroll = document.querySelector<HTMLElement>('.turn-detail')?.scrollTop ?? 0;
  const item = await client.get(id);
  if (generationTicket !== generation || syncTicket !== sessionSync || environment?.id !== id || routeFromLocation()?.kind !== 'session') return;
  environment = item;
  ...
  requestAnimationFrame(() => {const detail = document.querySelector<HTMLElement>('.turn-detail'); if (detail) detail.scrollTop = turnScroll;});
}
async function refreshSelectedActivity(changes: ActivityEvent[]) {
  if (!environment || !changes.some(event => event.environment === environment?.id)) return;
  await refreshCurrentSession();
}
function scheduleActivity(delay = 1000) {
  clearTimeout(activityTimer); activityTimer = window.setTimeout(() => void activityTick(), delay);
}
async function activityTick() {
  try {
    ...
    activityCursor = Math.max(activityCursor, page.cursor); activityFailures = 0;
    if (changes.length) {await list(); await refreshSelectedActivity(changes);}
...
async function attach(id: string, tab: SessionTab = 'overview', updateLocation = true) {
  ...
  const rangeEnd = el<HTMLInputElement>('compare-turn-end');
  delete rangeEnd.dataset.initialized; rangeEnd.value = '1';
  el<HTMLInputElement>('compare-turn-start').value = '1';
  renderHeader(item, tab);
  if (updateLocation) setLocation(sessionPath(item.id, tab));
  const perspective = el<HTMLSelectElement>('perspective');
  const previous = perspective.value;
  perspective.replaceChildren(new Option('Everyone', ''));
  for (const participant of item.participants) perspective.add(new Option(participant, participant));
  if (item.participants.includes(previous)) perspective.value = previous;
...
async function loadEvents() {
  ...
  while (true) {
    ...
    if (ticket !== generation) return;
    cursor = page.cursor; events = [...events, ...page.events]; received += page.events.length;
    if (page.events.length < 200) break;
  }
  if (received || !events.length) renderTimeline();
  renderModeSummary();
  el('timeline-note').textContent = `${events.length} recorded events, grouped by revision.`;
  el<HTMLButtonElement>('load-more').hidden = true;
}
async function download(path: string, filename: string) {
  ...
  const link = document.createElement('a'); link.href = URL.createObjectURL(await response.blob()); link.download = filename; link.click();
  setTimeout(() => URL.revokeObjectURL(link.href), 1000);
}
async function runCompare(ids: string[], requestedView: ComparisonViewState = {}) {
  if (ids.length < 2) throw new Error('Select at least two environments.');
  const loadSeries = (startTurn = 1, endTurn?: number) => Promise.all(ids.map(id =>
    client.turnSeries(id, {startTurn, endTurn, maxPoints: 300})));
  ...
  const sessions = (series: TurnSeriesResponse[]) => {
  ...
  renderComparison(result, sessions(projections), async (startTurn, endTurn) =>
    sessions(await loadSeries(startTurn, endTurn)), initialView, view =>
      setLocation(selectionPath('/compare', ids, view), true));
  if (hasRequestedWindow) setLocation(selectionPath('/compare', ids, initialView), true);
  el('comparison').textContent = json(result);
  el('compare-results').scrollIntoView({behavior: 'smooth', block: 'nearest'});
}

// Rendering
function homePageSlice<T>(entries: T[]) {
  const requested = Number(el<HTMLSelectElement>('home-page-size').value);
  ...
  el<HTMLButtonElement>('home-page-previous').disabled = homePage === 1;
  el<HTMLButtonElement>('home-page-next').disabled = homePage === pages;
...
function renderList() {
  ...
  const restoreTableScroll = () => requestAnimationFrame(() => {
    holder.scrollTop = tableScroll.top; holder.scrollLeft = tableScroll.left;
  });
  ...
  if (snapshot) {
    const parts: string[] = [];
    if (snapshot.summary.running) parts.push(`${snapshot.summary.running} running`);
    if (snapshot.summary.queued) parts.push(`${snapshot.summary.queued} queued`);
    if (snapshot.summary.failed) parts.push(`${snapshot.summary.failed} failed`);
    activitySummary.textContent = parts.join(' · ');
    activitySummary.hidden = parts.length === 0;
  } else {
    activitySummary.hidden = true;
  }
  const status = el<HTMLSelectElement>('session-status');
  ...
  syncDropdown(status);
  const query = el<HTMLInputElement>('session-search').value.trim().toLowerCase();
  ...
  const sort = el<HTMLSelectElement>('session-sort').value;
  rows.sort((a, b) => sort === 'oldest' ? a.id.localeCompare(b.id) : b.id.localeCompare(a.id));
  if (snapshot) {renderActivityRows(holder, snapshot, query, status.value, sort); restoreTableScroll(); return;}
  ...
  for (const item of homePageSlice(rows)) {
    const row = text('div', '', 'home-session-row'); row.dataset.sessionId = item.id; row.dataset.turnCount = String(item.revision);
    ...
    check.onchange = () => {
      ...
      persistSelection(); renderHomeSelection();
    };
    ...
    session.append(button, text('small', `${item.parent ? 'Branch' : 'Original'} · ${shortId(item.id)}`));
    row.append(check, session, text('span', 'Session', 'home-session-value'),
      text('span', sessionStatus(item.status), 'session-status'), text('span', '1', 'home-session-value'),
      text('span', formatCompactCount(item.revision), 'home-session-value'));
    holder.append(row);
  }
  if (!rows.length) holder.append(text('p', catalog.length ? 'No environment sessions match these filters.' : 'No environment sessions found for this credential.', 'home-empty muted'));
  renderHomeSelection();
  renderCompareSessionList();
  restoreTableScroll();
}

function activitySearch(session: ActivitySession) {
  return [session.scenario_id, session.id, session.status, session.participants.join(' '), session.latest_activity ?? '', session.failure ?? '', session.environment.implementation ?? ''].join(' ').toLowerCase();
}
function activityMatches(session: ActivitySession, query: string, status: string) {
  return (!query || activitySearch(session).includes(query)) && (!status || statusFilter(session.status) === status);
}
const scenarioKey = (experiment: string, scenario: string) => `${experiment}:${scenario}`;
const totalRecordedTurns = (sessions: ActivitySession[]) =>
  sessions.reduce((total, session) => total + session.current_turn, 0);
function scenarioSearch(scenario: ActivityScenario) {
  return [scenario.id, scenario.status, scenario.latest_activity ?? '', JSON.stringify(scenario.input), JSON.stringify(scenario.metadata)].join(' ').toLowerCase();
}
function childActivityRow(session: ActivitySession, scenario: ActivityScenario) {
  const row = text('div', '', 'home-session-row scenario-session');
  row.dataset.sessionId = session.id; row.dataset.turnCount = String(session.current_turn);
  const check = document.createElement('input'); check.type = 'checkbox'; check.checked = selected.has(session.id);
  const name = `${scenarioTitle(scenario)} · Trial ${session.trial}`;
  check.setAttribute('aria-label', `Select ${name}`);
  check.onchange = () => {
    check.checked ? selected.add(session.id) : selected.delete(session.id);
    persistSelection(); renderList();
  };
  ...
  identity.append(button, text('small', `Session · ${shortId(session.id)}`));
  row.append(check, identity, text('span', 'Session', 'home-session-value'),
    text('span', sessionStatus(session.status), 'session-status'), text('span', '1', 'home-session-value'),
    text('span', formatCompactCount(session.current_turn), 'home-session-value'));
  return row;
}
function standaloneActivityRow(session: ActivitySession) {
  ...
  button.onclick = () => void attempt(() => attach(session.id));
  identity.append(button, text('small', `Standalone · ${shortId(session.id)}`));
  row.append(check, identity, text('span', 'Session', 'home-session-value'),
    text('span', sessionStatus(session.status), 'session-status'), text('span', '1', 'home-session-value'),
    text('span', formatCompactCount(session.current_turn), 'home-session-value'));
  return row;
}
function renderActivityRows(holder: HTMLElement, snapshot: ActivitySnapshot, query: string, status: string, sort: string) {
  const header = text('div', '', 'home-session-row home-session-columns');
  for (const label of ['', 'Name', 'Type', 'Status', 'Sessions', 'Turns']) header.append(text('span', label));
  holder.append(header);
  const hasFilter = Boolean(query || status);
  const experiments = snapshot.experiments.map(item => {
    const parent = (!query || [item.name, item.status, item.latest_activity ?? ''].join(' ').toLowerCase().includes(query)) && (!status || statusFilter(item.status) === status);
    const scenarios = item.scenarios.map(scenario => ({
      item: scenario,
      matches: scenario.sessions.filter(session => activityMatches(session, query, status)),
      parent: (!query || scenarioSearch(scenario).includes(query)) && (!status || statusFilter(scenario.status) === status),
    })).filter(group => !hasFilter || group.parent || group.matches.length);
    return {item, parent, scenarios};
  }).filter(group => group.parent || group.scenarios.length);
  ...
  const visible = experiments.reduce((count, group) => count + (hasFilter && !group.parent
    ? group.scenarios.flatMap(scenario => scenario.matches).length
    : group.scenarios.reduce((sum, scenario) => sum + scenario.item.sessions.length, 0)), 0) + standalone.length;
  const page = homePageSlice([
    ...experiments.map(group => ({kind: 'experiment' as const, id: group.item.id})),
    ...standalone.map(session => ({kind: 'session' as const, id: session.id})),
  ]);
  ...
  for (const {item, scenarios} of experiments) {
    if (!visibleExperiments.has(item.id)) continue;
    const matchingSessions = scenarios.flatMap(group => group.matches);
    const row = text('div', '', 'home-session-row experiment-parent'); row.dataset.experimentId = item.id;
    const experimentCheck = document.createElement('input'); experimentCheck.type = 'checkbox';
    ...
    experimentCheck.onchange = () => {
      for (const session of item.sessions) experimentCheck.checked ? selected.add(session.id) : selected.delete(session.id);
      if (experimentCheck.checked) {
        expandedExperiments.add(item.id);
        for (const scenario of item.scenarios) expandedScenarios.add(scenarioKey(item.id, scenario.id));
      }
      persistSelection(); renderList();
    };
    const identity = text('span', '', 'home-session-cell experiment-identity');
    const name = text('strong', item.name, 'experiment-name');
    const forcedOpen = Boolean(hasFilter && scenarios.length);
    const open = expandedExperiments.has(item.id) || forcedOpen;
    const disclosure = text('button', open ? 'Hide scenarios' : `View ${item.scenarios.length} scenarios`, 'experiment-disclosure');
    const childId = `experiment-${item.id}-scenarios`;
    disclosure.setAttribute('aria-expanded', String(open)); disclosure.setAttribute('aria-controls', childId);
    ...
    disclosure.onkeydown = event => {
      if (event.key === 'ArrowRight' && !expandedExperiments.has(item.id)) {event.preventDefault(); expandedExperiments.add(item.id); renderList();}
      if (event.key === 'ArrowLeft' && expandedExperiments.has(item.id)) {event.preventDefault(); expandedExperiments.delete(item.id); renderList();}
    };
    const identityCopy = text('span', '', 'experiment-identity-copy');
    identityCopy.append(name, text('small', hasFilter && matchingSessions.length ? `${matchingSessions.length} matching of ${item.sessions.length} sessions` : `${item.scenarios.length} scenarios · ${item.sessions.length} environment sessions`));
    identity.append(disclosure, identityCopy);
    row.onclick = event => {
      if ((event.target as Element).closest('input, button, a')) return;
      toggleExperiment();
    };
    row.append(experimentCheck, identity, text('span', 'Experiment', 'home-session-value'),
      text('span', sessionStatus(item.status), 'session-status'), text('span', String(item.sessions.length), 'home-session-value'),
      text('span', formatCompactCount(totalRecordedTurns(item.sessions)), 'home-session-value'));
    holder.append(row);
    if (open) {
      const group = text('div', '', 'experiment-children'); group.id = childId;
      for (const scenarioGroup of scenarios) {
        const scenario = scenarioGroup.item;
        const key = scenarioKey(item.id, scenario.id);
        const scenarioRow = text('div', '', 'home-session-row scenario-row');
        scenarioRow.dataset.scenarioId = scenario.id;
        const scenarioCheck = document.createElement('input'); scenarioCheck.type = 'checkbox';
        const selectedSessions = scenario.sessions.filter(session => selected.has(session.id)).length;
        scenarioCheck.checked = Boolean(scenario.sessions.length) && selectedSessions === scenario.sessions.length;
        scenarioCheck.indeterminate = selectedSessions > 0 && selectedSessions < scenario.sessions.length;
        scenarioCheck.setAttribute('aria-label', `Select all sessions in ${scenarioTitle(scenario)}`);
        scenarioCheck.onclick = event => event.stopPropagation();
        scenarioCheck.onchange = () => {
          for (const session of scenario.sessions) scenarioCheck.checked ? selected.add(session.id) : selected.delete(session.id);
          if (scenarioCheck.checked) expandedScenarios.add(key);
          persistSelection(); renderList();
        };
        const scenarioIdentity = text('span', '', 'home-session-cell scenario-identity');
        const scenarioName = text('strong', scenarioTitle(scenario), 'scenario-name');
        const scenarioForcedOpen = Boolean(hasFilter && scenarioGroup.matches.length);
        const scenarioOpen = expandedScenarios.has(key) || scenarioForcedOpen;
        const scenarioDisclosure = text('button', scenarioOpen ? 'Hide sessions' : `View ${scenario.sessions.length} sessions`, 'scenario-disclosure');
        const sessionsId = `scenario-${item.id}-${scenario.id}-sessions`;
        scenarioDisclosure.setAttribute('aria-expanded', String(scenarioOpen)); scenarioDisclosure.setAttribute('aria-controls', sessionsId);
        const toggleScenario = () => {expandedScenarios.has(key) ? expandedScenarios.delete(key) : expandedScenarios.add(key); renderList();};
        scenarioDisclosure.onclick = event => {event.stopPropagation(); toggleScenario();};
        scenarioDisclosure.onkeydown = event => {
          if (event.key === 'ArrowRight' && !expandedScenarios.has(key)) {event.preventDefault(); expandedScenarios.add(key); renderList();}
          if (event.key === 'ArrowLeft' && expandedScenarios.has(key)) {event.preventDefault(); expandedScenarios.delete(key); renderList();}
        };
        const scenarioCopy = text('span', '', 'scenario-identity-copy');
        scenarioCopy.append(scenarioName, text('small', scenarioForcedOpen ? `${scenarioGroup.matches.length} matching of ${scenario.sessions.length} sessions` : 'Frozen configuration'));
        scenarioIdentity.append(scenarioDisclosure, scenarioCopy);
        scenarioRow.onclick = event => {if (!(event.target as Element).closest('input, button, a')) toggleScenario();};
        scenarioRow.append(scenarioCheck, scenarioIdentity, text('span', 'Scenario', 'home-session-value'),
          text('span', sessionStatus(scenario.status), 'session-status'), text('span', String(scenario.sessions.length), 'home-session-value'),
          text('span', formatCompactCount(totalRecordedTurns(scenario.sessions)), 'home-session-value'));
        group.append(scenarioRow);
        if (scenarioOpen) {
          const sessions = text('div', '', 'scenario-sessions'); sessions.id = sessionsId;
          const children = hasFilter && !scenarioGroup.parent ? scenarioGroup.matches : scenario.sessions;
          for (const session of children) sessions.append(childActivityRow(session, scenario));
  ...
  for (const session of standalone)
    if (visibleStandalone.has(session.id)) holder.append(standaloneActivityRow(session));
  const visibleCount = el('visible-session-count');
  visibleCount.textContent = `${visible} visible`;
  visibleCount.hidden = !query && !status;
  if (!experiments.length && !standalone.length) holder.append(text('p', 'No environment sessions match these filters.', 'home-empty muted'));
  renderHomeSelection(); renderCompareSessionList();
}
function renderHomeSelection() {
  ...
  const button = el<HTMLButtonElement>('compare');
...
function setComparisonPickerVisible(visible: boolean) {
  el('compare-session-list').hidden = !visible;
  el<HTMLButtonElement>('run-comparison').hidden = !visible;
}
function showCompareSessions(updateLocation = true, showPicker = selected.size < 2) {
  ...
  if (updateLocation) setLocation(selectionPath('/compare', [...selected]));
  renderCompareSessionList();
}
function renderHeader(item: Environment, tab: SessionTab = 'overview') {
  ...
  for (const [label, value] of [['Revision', String(item.revision)], ['Participants', String(item.participants.length)], ['Spent', formatCost(item.spent_micros)], ['Reserved', formatCost(item.reserved_micros)]]) {
    const stat = text('div', '', 'stat'); stat.append(text('strong', value), text('span', label)); stats.append(stat);
  }
  const spec = item.environment as Record<string, Json>;
...
function renderFrozenSummary(item: Environment) {
  const holder = el('frozen-summary'); holder.replaceChildren();
...
function renderModeSummary() {
  const holder = el('mode-summary'); holder.replaceChildren();
...
function renderTimeline() {
  ...
  const expanded = new Set(Array.from(holder.querySelectorAll<HTMLDetailsElement>('details[open]')).map(node => node.dataset.eventId));
  holder.replaceChildren();
  const perspective = el<HTMLSelectElement>('perspective').value;
  const turns = buildTimeline(filterEvents(events, perspective, el<HTMLSelectElement>('kind').value), environment?.participants ?? []);
  for (const turn of turns) holder.append(renderTurn(turn, perspective));
  if (!turns.length) holder.append(text('p', 'No recorded events match this perspective.', 'muted'));
  for (const node of holder.querySelectorAll<HTMLDetailsElement>('details[data-event-id]')) node.open = expanded.has(node.dataset.eventId);
  renderInspect();
}
function inspectTurns() {
  return buildTimeline(events, environment?.participants ?? []).filter(hasActivity);
}
function slotValue(turn: Turn, participant: string, slot: Slot) {
  const event = turn.participants[participant]?.[slot];
  return event ? cellText(event, slot) : MISSING[slot];
}
function renderInspect() {
  const turns = inspectTurns();
  if (selectedTurnIndex < 0 || selectedTurnIndex >= turns.length) selectedTurnIndex = Math.max(0, turns.length - 1);
  const list = el('turn-list'); list.replaceChildren();
  ...
  const previous = el<HTMLButtonElement>('previous-turn'), next = el<HTMLButtonElement>('next-turn');
  previous.disabled = selectedTurnIndex === 0; next.disabled = selectedTurnIndex === turns.length - 1;
  previous.onclick = () => {
    if (selectedTurnIndex > 0) {selectedTurnIndex -= 1; selectedParticipant = null; renderInspect();}
  };
  next.onclick = () => {
    if (selectedTurnIndex < turns.length - 1) {selectedTurnIndex += 1; selectedParticipant = null; renderInspect();}
  };

  const progression = el('turn-progression'); progression.replaceChildren();
  ...
  for (const [label, value] of [
    ...
    step.append(text('strong', value)); progression.append(step);
  }
  el('starting-state-value').textContent = `Revision ${turn.revision}${
    Object.keys(turn.shared).length ? ` · ${scalars(turn.shared)}` : ''}`;

  const participants = el('participant-evidence-list'); participants.replaceChildren();
...
function renderSelectedEvidence(turn: Turn) {
  const holder = el('selected-evidence'); holder.replaceChildren();
  if (!selectedParticipant) {
    holder.append(text('p', 'No participant evidence was recorded for this turn.')); return;
  }
  const heading = text('div', '', 'selected-evidence-heading');
  heading.append(text('h3', participantName(selectedParticipant)),
    text('span', `Turn ${selectedTurnIndex + 1} · Revision ${turn.revision}`, 'muted'));
  holder.append(heading);
  const grid = text('div', '', 'evidence-summary-grid');
  const labels: Record<Slot, string> = {observation: 'Observation', attempted: 'Action', executed: 'Outcome'};
...
function renderProgression() {
  ...
  const select = el<HTMLSelectElement>('turn-series-select');
  ...
  if (rewardSeries.length) select.add(new Option('Participant Reward', 'reward'));
  for (const series of metricSeries) select.add(new Option(series.label, series.id));
  for (const series of signalSeries) select.add(new Option(series.label, series.id));
  for (const series of activitySeries) select.add(new Option(series.label, series.id));
  for (const series of findingSeries) select.add(new Option(series.label, series.id));
  const available = new Set(Array.from(select.options).map(option => option.value));
  const metricDefault = [...metricSeries].sort((a, b) =>
    new Set(b.points.map(point => point.value)).size - new Set(a.points.map(point => point.value)).size
  ...
  const participant = el<HTMLSelectElement>('turn-series-participant');
  const previousParticipant = participant.value;
  participant.replaceChildren(new Option('All Participants', ''));
  for (const name of environment?.participants ?? []) participant.add(new Option(participantName(name), name));
  participant.value = (environment?.participants ?? []).includes(previousParticipant) ? previousParticipant : '';
  syncDropdown(select); syncDropdown(participant);
  ...
  const start = el<HTMLInputElement>('compare-turn-start');
  const end = el<HTMLInputElement>('compare-turn-end');
  const maximum = Math.max(1, turns.length);
  start.max = String(maximum); end.max = String(maximum);
  if (!end.dataset.initialized && turns.length) {end.value = String(maximum); end.dataset.initialized = 'true';}
  if (Number(start.value) < 1 || Number(start.value) > maximum) start.value = '1';
  if (Number(end.value) < 1 || Number(end.value) > maximum) end.value = String(maximum);
  let first = Math.min(Number(start.value), Number(end.value));
  let last = Math.max(Number(start.value), Number(end.value));
  if (!turns.length) {first = 0; last = 0;}
  ...
  selection.dataset.first = String(first); selection.dataset.last = String(last); selection.dataset.maximum = String(maximum);
  ...
  const chart = document.getElementById('turn-series-chart') as unknown as SVGSVGElement;
  ...
  const svg = (tag: string, attributes: Record<string, string>, value = '') => {
  ...
  if (minimum === maximumValue) {minimum -= Math.abs(minimum || 1) * .1; maximumValue += Math.abs(maximumValue || 1) * .1;}
  const x = (turn: number) => left + ((turn - first) / Math.max(1, last - first)) * (right - left);
  const y = (value: number) => bottom - ((value - minimum) / Math.max(Number.EPSILON, maximumValue - minimum)) * (bottom - top);
  ...
  for (const ratio of [0, .5, 1]) {
    ...
    chart.append(svg('text', {x: String(left - 10), y: String(lineY + 4), class: 'series-axis-label', 'text-anchor': 'end'},
      formatValue(minimum + ratio * (maximumValue - minimum))));
  }
  ...
  visibleSeries.forEach((series, seriesIndex) => {
    const segments: typeof series.points[] = [];
    for (const point of series.points) {
      const segment = segments.at(-1);
      if (!segment?.length || point.turn !== segment.at(-1)!.turn + 1) segments.push([point]);
  ...
  if (!values.length) chart.append(svg('text', {x: String((left + right) / 2), y: '125', class: 'series-empty', 'text-anchor': 'middle'}, 'No recorded values in this range'));
  ...
  const legend = el('turn-series-legend'); legend.replaceChildren();
  for (const series of visibleSeries) legend.append(text('span', series.label));

  const markerHolder = el('turn-series-markers'); markerHolder.replaceChildren();
  const revisionTurns = new Map(turns.map((turn, index) => [turn.committed?.revision, index + 1]));
  const markers: {turn: number; label: string}[] = [];
  for (const event of events) {
    const turn = revisionTurns.get(event.revision);
    if (!turn || turn < first || turn > last) continue;
    if (event.kind === 'checkpoint.committed') markers.push({turn, label: 'Checkpoint'});
    if (event.kind === 'report') markers.push({turn, label: 'Score Report'});
  }
  for (const [index, turn] of turns.entries()) {
    if (index + 1 < first || index + 1 > last) continue;
    if (turn.committed?.terminated) markers.push({turn: index + 1, label: 'Terminated'});
    else if (turn.committed?.truncated) markers.push({turn: index + 1, label: 'Truncated'});
  }
  for (const series of findingSeries) {
    for (const point of series.points) if (point.turn >= first && point.turn <= last) {
      markers.push({turn: point.turn, label: `${series.label} (${formatValue(point.value)})`});
    }
  }
  if (markers.length) for (const marker of markers) markerHolder.append(text('span', `Turn ${marker.turn} · ${marker.label}`));
  else markerHolder.append(text('span', 'No score reports, checkpoints, or terminal events in this range.', 'muted'));

  const difference = el('turn-difference'); difference.replaceChildren();
  if (!turns.length) {
  ...
  for (const series of selectedSeries) {
    ...
    row.append(text('strong', series.label),
      text('span', `Turn ${turnA}\n${firstValue === undefined ? 'Not recorded' : formatValue(firstValue)}`),
      text('span', `Turn ${turnB}\n${lastValue === undefined ? 'Not recorded' : formatValue(lastValue)}`),
  ...
  if (!changed) evidenceChanges.append(text('p', 'No participant evidence changed between these turns.', 'muted'));
  difference.append(evidenceChanges);
}
function renderCompareSessionList() {
  const holder = el('compare-session-list'); holder.replaceChildren();
  for (const item of catalog) {
    ...
    check.onchange = () => {
      check.checked ? selected.add(item.id) : selected.delete(item.id);
      persistSelection(); renderCompareSessionList(); renderModeSummary();
    };
    label.append(check, copy); holder.append(label);
  }
}
function renderTurn(turn: Turn, perspective: string) {
  ...
  if (turn.committed?.terminated) head.append(text('span', 'terminated', 'tag'));
  else if (turn.committed?.truncated) head.append(text('span', 'truncated', 'tag'));
  if (Object.keys(turn.shared).length) head.append(text('span', scalars(turn.shared), 'shared'));
  section.append(head);
...
function renderParticipantRow(name: string, slots: Turn['participants'][string], perspective: string) {
  const row = text('div', '', 'turn-row'); row.append(text('span', name, 'participant'));
  const empty = SLOTS.every(slot => !slots[slot]);
  if (empty) {row.append(text('span', perspective && name !== perspective ? 'not visible from this perspective' : 'no events in this turn', 'cell missing')); return row;}
  for (const slot of SLOTS) {
    const event = slots[slot];
    if (!event) {row.append(text('span', MISSING[slot], 'cell missing')); continue;}
    const cell = details(cellText(event, slot), payload(event), 'cell', `${event.environment}:${event.seq}`);
    if (slot === 'executed' && typeof event.payload.reward === 'number') cell.classList.add(event.payload.reward >= 0 ? 'positive' : 'negative');
    row.append(cell);
  }
  return row;
}
function payload(event: EvidenceEvent) {
...
function renderNote(event: EvidenceEvent) {
...
function evaluationValue(value: Json) {
...
function renderReports(value: ReportEnvelope[]) {
  const holder = el('reports'); holder.replaceChildren();
  if (!value.length) {holder.append(text('p', 'No scores or findings recorded.', 'muted')); return;}
  ...
  const metrics = text('section', '', 'evaluation-metrics'); metrics.id = 'evaluation-metrics';
  const heading = text('div', '', 'evaluation-metric evaluation-metric-heading');
  for (const label of ['Metric', 'Start', 'Latest', 'Change']) heading.append(text('span', label));
  ...
  for (const metric of summary.metrics) {
    ...
    row.append(name, text('span', evaluationValue(metric.first)), text('span', evaluationValue(metric.latest)),
      text('span', metric.change === null ? '—' : `${metric.change > 0 ? '+' : ''}${evaluationValue(metric.change)}`));
    metrics.append(row);
  }
  if (summary.metrics.length) holder.append(metrics);
  else holder.append(text('p', 'No metrics recorded.', 'muted'));

  const findings = text('section', '', 'evaluation-findings');
  findings.append(text('h3', 'Findings'));
  if (!summary.findings.length) findings.append(text('p', 'No findings recorded.', 'muted'));
  for (const finding of summary.findings) {
    const row = text('article', '', 'evaluation-finding');
    row.append(text('strong', `${participantName(finding.category)} · ${participantName(finding.status)}`),
      text('span', [finding.turn ? `Turn ${finding.turn}` : 'Turn unavailable', participantName(finding.participant), finding.rule].join(' · ')),
      text('p', finding.judgment));
    findings.append(row);
  }
  holder.append(findings);
  if (summary.uncertainties.length) holder.append(text('p', summary.uncertainties.at(-1)!, 'evaluation-uncertainty'));
}
function renderReportHistory(value: ReportEnvelope[]) {
  const holder = el('report-history'); holder.replaceChildren();
  if (!value.length) {holder.append(text('p', 'No reports recorded.', 'muted')); return;}
  ...
  for (const [label, factValue] of [
    ...
    const fact = text('div', ''); fact.append(text('dt', label), text('dd', factValue)); facts.append(fact);
  }
  ...
  if (latestMetrics.childElementCount) latestCard.append(latestMetrics);
  ...
  latestSection.append(latestCard); holder.append(latestSection);
  ...
  for (const record of records) {
    const block = document.createElement('details'); block.className = 'report-record';
    const summary = text('summary', '', 'report-record-summary');
    const heading = text('span', '', 'report-record-heading');
    heading.append(text('strong', `Revision ${record.revision}`),
      text('small', `${record.report.scorer}@${record.report.version} · Evidence ${record.report.evidence_cursor}`));
    ...
    summary.append(heading, summaryMetrics); block.append(summary);
    ...
    for (const [label, factValue] of [
      ...
      const fact = text('div', ''); fact.append(text('dt', label), text('dd', factValue)); recordFacts.append(fact);
    }
    body.append(recordFacts);
    if (record.report.findings?.length) {
      const findings = text('section', '', 'report-record-findings'); findings.append(text('h4', 'Findings'));
      for (const finding of record.report.findings) {
        const row = text('article', '', 'evaluation-finding');
        row.append(text('strong', `${participantName(finding.category)} · ${participantName(finding.status)}`),
          text('span', `${participantName(finding.participant)} · ${finding.rule}`), text('p', finding.judgment));
        findings.append(row);
      }
    ...
    if (record.report.uncertainty) {
      ...
      uncertainty.append(text('strong', 'Uncertainty'), text('p', record.report.uncertainty)); body.append(uncertainty);
    }
    body.append(details('Raw', text('pre', json(record)))); block.append(body); history.append(block);
  }
  holder.append(history);
}

function renderReportProgression(holder: HTMLElement, recordsNewestFirst: ReportEnvelope[]) {
  const records = [...recordsNewestFirst].reverse();
  const series = new Map<string, Array<{revision: number; value: number}>>();
  for (const record of records) {
    for (const [key, value] of Object.entries(record.report.metrics ?? {})) {
      if (typeof value !== 'number' || !Number.isFinite(value)) continue;
      const points = series.get(key) ?? []; points.push({revision: record.revision, value}); series.set(key, points);
    }
  }
  ...
  for (const [key, points] of series) {
    ...
    svg.setAttribute('viewBox', '0 0 420 190'); svg.setAttribute('role', 'img'); svg.setAttribute('aria-labelledby', titleId);
    const svgNode = (tag: string, attributes: Record<string, string>, value = '') => {
      const node = document.createElementNS('http://www.w3.org/2000/svg', tag);
    ...
    if (minimum === maximum) {minimum -= Math.abs(minimum || 1) * .1; maximum += Math.abs(maximum || 1) * .1;}
    const x = (revision: number) => left + ((revision - minimumRevision) / Math.max(1, maximumRevision - minimumRevision)) * (right - left);
    const y = (metric: number) => bottom - ((metric - minimum) / Math.max(Number.EPSILON, maximum - minimum)) * (bottom - top);
    for (const ratio of [0, .5, 1]) {
      const lineY = bottom - ratio * (bottom - top);
      svg.append(svgNode('line', {x1: String(left), x2: String(right), y1: String(lineY), y2: String(lineY), class: 'comparison-grid-line'}));
      svg.append(svgNode('text', {x: String(left - 8), y: String(lineY + 4), class: 'comparison-axis-label', 'text-anchor': 'end'},
        chartValue(minimum + ratio * (maximum - minimum))));
    }
    ...
    if (path) svg.append(svgNode('path', {d: points.length === 1 ? `${path} l 1 0` : path, class: 'comparison-line'}));
    ...
    const legendItem = text('span', ''); legendItem.append(text('i', ''), document.createTextNode(`${points.length} reported revisions`));
    legend.append(legendItem, text('small', `Latest ${chartValue(points.at(-1)!.value)}${definition?.unit ? ` ${definition.unit}` : ''}`));
    chart.append(legend); grid.append(chart);
  }
  progression.append(grid); section.append(progression); holder.append(section);
}
type ComparisonSeriesPoint = {turn: number; value: number};
type ComparisonMetricSeries = {points: ComparisonSeriesPoint[]; sourcePoints: number; downsampled: boolean};
type ComparisonSessionSeries = {
  environment: string; title: string; scenario: string; trial: string; totalTurns: number;
  metrics: Record<ComparisonMetricId, ComparisonMetricSeries>;
};
type ComparisonSeriesLoader = (startTurn: number, endTurn: number) => Promise<ComparisonSessionSeries[]>;
type ComparisonViewChange = (view: ComparisonViewState) => void;
const comparisonMetricDefinitions: Array<{id: ComparisonMetricId; label: string; unit: string}> = [
...
function comparisonSessionSeries(item: Comparison['environments'][number], projection?: TurnSeriesResponse): ComparisonSessionSeries {
  ...
  const metric = (id: string): ComparisonMetricSeries => {
...
const chartValue = (value: number) => Number(value.toPrecision(6)).toString();

function renderComparisonVisualizations(
  holder: HTMLElement, initialSessions: ComparisonSessionSeries[], loadRange: ComparisonSeriesLoader,
  initialView: ComparisonViewState, onViewChange: ComparisonViewChange,
) {
  let sessions = initialSessions;
  const available = comparisonMetricDefinitions.filter(metric =>
    sessions.some(session => session.metrics[metric.id].points.length));
  if (!available.length) return;
  const finalValue = (session: ComparisonSessionSeries, metric: ComparisonMetricId) =>
    session.metrics[metric].points.at(-1)?.value;
  const defaultMetric = [...available].sort((a, b) =>
    new Set(sessions.map(session => finalValue(session, b.id)).filter(value => value !== undefined)).size
  ...
  controls.setAttribute('role', 'group'); controls.setAttribute('aria-label', 'Comparison metric');
  ...
  const rangeValue = (label: string, value: number) => {
    const holder = text('div', '');
    const output = text('dd', `Turn ${value}`);
    holder.append(text('dt', label), output); return {holder, output};
  };
  ...
  const resetRange = text('button', 'Full Range', 'text-button') as HTMLButtonElement;
  resetRange.type = 'button';
  rangeActions.append(rangeValues, resetRange); rangeLabels.append(rangeCopy, rangeActions);
  ...
  const rangeInput = (label: string, value: number) => {
    const input = document.createElement('input');
    input.type = 'range'; input.min = '1'; input.max = String(maximumTurn); input.value = String(value);
    input.setAttribute('aria-label', `${label} turn`); return input;
  };
  const startControl = rangeInput('Start', rangeStart), endControl = rangeInput('End', rangeEnd);
  rangeTrack.append(rangeSelection, startControl, endControl); range.append(rangeLabels, rangeTrack);
  ...
  const updateRangeLabels = () => {
    ...
    rangeSelection.dataset.first = String(rangeStart); rangeSelection.dataset.last = String(rangeEnd);
  ...
  const render = () => {
    ...
    const scenarios = new Map<string, ComparisonSessionSeries[]>();
    for (const session of visible) {
      const group = scenarios.get(session.scenario) ?? [];
      group.push(session); scenarios.set(session.scenario, group);
    }
    for (const [scenario, group] of scenarios) {
      ...
      group.forEach((session, index) => {
        ...
        if (position > .72) marker.classList.add('is-end');
      ...
      row.append(track); finalPlot.append(row);
    }
    ...
    for (const [scenario, group] of scenarios) {
      ...
      const left = 42, right = 405, top = 14, bottom = 154;
      const x = (turn: number) => left + ((turn - rangeStart) / Math.max(1, rangeEnd - rangeStart)) * (right - left);
      const y = (value: number) => bottom - ((value - seriesMinimum) / Math.max(Number.EPSILON, seriesMaximum - seriesMinimum)) * (bottom - top);
      for (const ratio of [0, .5, 1]) {
        const lineY = bottom - ratio * (bottom - top);
        svg.append(svgNode('line', {x1: String(left), x2: String(right), y1: String(lineY), y2: String(lineY), class: 'comparison-grid-line'}));
        svg.append(svgNode('text', {x: String(left - 8), y: String(lineY + 4), class: 'comparison-axis-label', 'text-anchor': 'end'},
          chartValue(seriesMinimum + ratio * (seriesMaximum - seriesMinimum))));
      }
      ...
      group.forEach((session, index) => {
        ...
        if (path) svg.append(svgNode('path', {d: points.length === 1 ? `${path} l 1 0` : path, class: `comparison-line trial-${index % 4}`}));
        for (const point of points) {
      ...
      if (new Set(signatures).size < signatures.length) legend.append(text('small', 'Overlapping trials share the same values.'));
      const sampled = group.filter(session => session.metrics[selectedMetric].downsampled);
      if (sampled.length) legend.append(text('small', `Extrema preserved from up to ${Math.max(...sampled.map(session => session.metrics[selectedMetric].sourcePoints))} recorded points.`));
      chart.append(legend); grid.append(chart);
    }
    progression.append(grid); body.append(progression);
  };

  for (const metric of available) {
    const button = text('button', metric.label, 'comparison-metric-button') as HTMLButtonElement;
    button.type = 'button'; button.dataset.metric = metric.id;
    button.onclick = () => {
  ...
  const refreshRange = () => void attempt(async () => {
    ...
    startControl.value = String(rangeStart); endControl.value = String(rangeEnd);
    updateRangeLabels();
    onViewChange({metric: selectedMetric, startTurn: rangeStart, endTurn: rangeEnd});
    ...
    try {
      ...
      sessions = loaded; render();
    } finally {
      if (request === rangeRequest) section.removeAttribute('aria-busy');
    }
  });
  const previewRange = () => {
    rangeStart = Math.min(Number(startControl.value), Number(endControl.value));
    rangeEnd = Math.max(Number(startControl.value), Number(endControl.value));
    startControl.value = String(rangeStart); endControl.value = String(rangeEnd); updateRangeLabels();
  };
  startControl.oninput = previewRange; endControl.oninput = previewRange;
  startControl.onchange = refreshRange; endControl.onchange = refreshRange;
  bindTurnRangeTrack(rangeTrack, rangeSelection, startControl, endControl, previewRange, refreshRange);
  resetRange.onclick = () => {
    startControl.value = '1'; endControl.value = String(maximumTurn); refreshRange();
  };
  heading.append(copy, controls); section.append(heading, range, body); holder.append(section); render();
}

function renderComparison(
  value: Comparison, sessions: ComparisonSessionSeries[], loadRange: ComparisonSeriesLoader,
  initialView: ComparisonViewState, onViewChange: ComparisonViewChange,
) {
  const result = value;

  const holder = el('compare-results'); holder.replaceChildren();
  ...
  for (const item of result.environments) {
    ...
    for (const [label, value] of [
      ...
      const fact = text('div', ''); fact.append(text('dt', label), text('dd', value)); facts.append(fact);
    }
    card.append(facts);
    for (const [key, changed] of Object.entries(item.interventions ?? {})) card.append(text('p', `${key.replaceAll('_', ' ')} set to ${typeof changed === 'object' ? JSON.stringify(changed) : String(changed)}.`, 'muted'));
    const metrics = item.latest_report?.report.metrics ?? {};
    for (const [key, metric] of Object.entries(metrics)) {
      const row = text('div', '', 'metric'); row.append(text('strong', typeof metric === 'object' ? JSON.stringify(metric) : String(metric)), text('span', key.replaceAll('_', ' '))); card.append(row);
    }
    if (!Object.keys(metrics).length) card.append(text('p', 'No Score Report Recorded', 'muted'));
    cards.append(card);
  }
  holder.append(cards);
  ...
  if (new Set(turnCounts).size > 1) {
    ...
    note.append(text('strong', 'Different Session Lengths'),
      text('p', `${minimumTurns}–${maximumTurns} recorded turns across these environment sessions. Charts use one shared turn axis; shorter sessions end at their actual final turn and are never padded.`));
    holder.append(note);
  }
...
function renderComparisonReports(holder: HTMLElement, result: Comparison) {
  ...
  const copy = text('div', '');
  copy.append(text('h3', 'Comparison Reports'),
    text('p', 'Aggregate score reports grouped by compatible scorer, version, metric definition, and cohort.'));
  ...
  if (result.uncertainty || result.design) {
    ...
    if (result.uncertainty) {
      const item = text('div', ''); item.append(text('strong', 'Uncertainty'), text('p', result.uncertainty)); context.append(item);
    }
    if (result.design) {
      const item = text('div', ''); item.append(text('strong', 'Comparison Basis'), text('p', result.design)); context.append(item);
    }
    section.append(context);
  }

  if (!groups.length) section.append(text('p', 'No compatible score report metrics were available.', 'muted'));
  ...
  for (const group of groups) {
    ...
    identity.append(text('h4', participantName(group.metric)),
      text('p', `${group.scorer}@${group.version} · ${participantName(group.kind)}`, 'muted'));
    cardHeading.append(identity, text('span', shortId(group.id), 'tag')); card.append(cardHeading);

    const aggregate = text('div', '', 'metric comparison-report-aggregate');
    aggregate.append(text('strong', group.summary ? chartValue(group.summary.mean_of_lineage_means) : 'Raw values only'),
      text('span', group.summary ? 'Mean of lineage means' : 'No compatible aggregate'));
    ...
    const disclosure = document.createElement('details'); disclosure.className = 'comparison-report-details';
    ...
    for (const label of ['Environment Session', 'Value', 'Revision', 'Status', 'Lineage']) valuesHeading.append(text('span', label));
    values.append(valuesHeading);
    for (const value of group.values) {
      const row = text('div', '', 'comparison-report-value');
      row.append(text('span', shortId(value.environment)), text('strong', chartValue(value.value)),
        text('span', String(value.report_revision)), text('span', sessionStatus(value.status)), text('span', shortId(value.lineage)));
      values.append(row);
    }
    if (!group.values.length) values.append(text('p', 'No environment-session values were reported.', 'muted'));
    disclosure.append(values, details('Raw metric group', text('pre', json(group)))); card.append(disclosure); grid.append(card);
  }
  ...
  if (result.warnings?.length) {
    ...
    for (const warning of result.warnings) warnings.append(text('p', warning));
...
async function connect(token: string) {
  ...
  el('access').hidden = true; el('site-header').hidden = true;
  el('local-access-error').hidden = true;
  document.body.classList.add('viewer-connected');
  el('workspace').hidden = false; el('connection').textContent = 'Connected';
  try {await list(); scheduleActivity();} catch {message('Participant credentials can attach by environment ID.');}
  await restoreRoute();
}
...
async function connectViewer() {
  ...
  try {
    ...
    if (!response.ok) throw new Error('Viewer configuration is unavailable.');
    const config = await response.json() as {authentication?: string};
    el('instance-summary').hidden = config.authentication !== 'local';
    if (config.authentication === 'credential') {
      el('access').hidden = false;
      return;
    }
    if (config.authentication !== 'local') throw new Error('Viewer authentication mode is unsupported.');
    const connection = await fetch('/local/connect', {method: 'POST', redirect: 'error', cache: 'no-store', signal: AbortSignal.timeout(10000)});
    if (!connection.ok) throw new Error('The local viewer could not establish researcher access.');
    await connect((await connection.json() as {token: string}).token);
  } catch {
    el('access').hidden = true;
...
for (const select of document.querySelectorAll<HTMLSelectElement>('select')) initializeDropdown(select);
void connectViewer();
...
el<HTMLSelectElement>('session-sort').onchange = resetHomePage;
el<HTMLSelectElement>('home-page-size').onchange = resetHomePage;
...
el<HTMLSelectElement>('turn-series-participant').onchange = renderProgression;
window.onpopstate = () => {
  if (!el('workspace').hidden) void attempt(restoreRoute);
};
bindTurnRangeTrack(
  el('turn-range-track'), el('turn-range-selection'),
  el<HTMLInputElement>('compare-turn-start'), el<HTMLInputElement>('compare-turn-end'), renderProgression,
);
setInterval(() => {if (environment && document.visibilityState === 'visible') void attempt(loadEvents);}, 3000);
