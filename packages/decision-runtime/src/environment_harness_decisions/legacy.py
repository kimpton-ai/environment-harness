...
class LegacyRun:
    ...
    def unresolved_charges(self):
        return tuple(
            ...
            if row.get("status") not in {"resolved", "settled", "rejected_before_dispatch"}
        )
    ...
    def unresolved_prepared_successors(self):
        return tuple(
            row
            for row in self.prepared_successors
            if row.get("status") in {"prepared", "unknown", "dispatching"}
        )

    def identity(self) -> dict[str, Any]:
        return {
            "reader": LEGACY_READER_VERSION,
...
def _database_paths(root: Path) -> tuple[Path, ...]:
    ...
    return tuple(
        sorted(
            ...
            if path.is_file() and not path.is_relative_to(root / ".decision-runtime")
        )
    )
