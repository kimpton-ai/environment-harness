...
def candidates():
    return (
        MotorCandidate(id="left", description="turn left", steps=(MotorStep(operation="turn", target={}),)),
    )
...
def test_legacy_surface_uses_mutable_transport_and_retains_evidence():
    ...
    selector.transport = lambda *args: {
        **response(),
        "answers": {"motor": {**response()["answers"]["motor"], "choice": "__abstain__"}},
    }
    assert (
        selector.select(
            {}, candidates(), maximum_cost_micros=10, cancel=threading.Event(), deadline=time.monotonic() + 2
        ).candidate_id
        is None
    )
