...
def inherit(store, db, environment, revision, original, limit):
    ...
    if kind in INHERITED:
        ...
        store.append(db, environment, revision, kind, json.loads(original['body']), audience, original['event_time'])
        return
    raw = encode(dict(original)).encode()
    if len(raw) <= limit:
        store.append(db, environment, revision, 'history.inherited', dict(original), audience, original['event_time'])
        return
    encoded = base64.b64encode(raw).decode('ascii')
    header = {
        'environment': original['environment'], 'seq': original['seq'], 'hash': original['hash'],
        'record_sha256': hashlib.sha256(raw).hexdigest(), 'encoding': 'base64-json-v1',
    }
    ...
    for part, offset in enumerate(range(0, len(encoded), capacity)):
        ...
        store.append(db, environment, revision, 'history.inherited.chunk', payload, audience, original['event_time'])
...
def reconstruct_inherited(events, *, strict=True):
    ...
    for event in events:
        ...
        if event['kind'] == 'history.inherited':
            ...
            key = (event['environment'], event['revision'], payload['environment'], payload['seq'], payload['hash'])
            if key not in groups:
                groups[key] = {'header': payload, 'chunks': {}, 'audience': event['audience']}
                order.append(key)
            group = groups[key]
            if (any(group['header'][k] != payload[k] for k in ('parts', 'encoding', 'record_sha256'))
                    or event['audience'] != group['audience']
                    or type(payload['part']) is not int or type(payload['parts']) is not int
    ...
    for item in order:
        ...
        try:
            ...
            if (hashlib.sha256(raw).hexdigest() != header['record_sha256']
