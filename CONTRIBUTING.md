...
To release, create a release branch after adding notes under `Unreleased`, then run `python scripts/release_version.py prepare --bump patch` (or `minor`/`major`). It moves the accumulated notes to a dated version and synchronizes `pyproject.toml`, `environment_harness.__version__`, `uv.lock`, the TypeScript package and lockfile, and the README example. Validate the proposed tag with `python scripts/release_version.py validate --release-tag vX.Y.Z`, run the release checks, and merge the release PR normally.
