...
To release, run the **Prepare release PR** workflow and choose patch, minor, or major. It moves the accumulated notes to a dated version and synchronizes `pyproject.toml`, `environment_harness.__version__`, `uv.lock`, the TypeScript package and lockfile, and the README example. Review and merge that PR normally. Release orchestration then creates the exact immutable `vX.Y.Z` tag at the reviewed merge commit, dispatches the attested publisher on that tag, and waits for independent `github-release` environment approval. Never rebuild different artifacts under an existing version.
