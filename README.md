...
`quickstart` writes durable evidence to `./environment-sessions`. `serve` opens that same store through an authenticated API and read-only viewer at `http://127.0.0.1:8765`. The loopback viewer receives read-only local access automatically; `--open` only launches the browser. Refreshing the page or restarting the server does not remove recorded progress. API clients still use an explicit credential from `environment-harness token`. The server binds only to your computer and does not deploy or publish the environment sessions. Press `Ctrl+C` to stop it.
