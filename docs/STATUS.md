...
EnvironmentHarness 0.1.0 focuses on local persistent sessions and recorded evidence. The synthetic examples exercise shared state, participant-specific observations, explicit checkpoints, isolated branches, agent execution and JSONL export. The viewer and command-agent example run locally without a model account.
