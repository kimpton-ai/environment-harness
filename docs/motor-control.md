...
`JevSelector` is opt-in and reads an explicit key or `TYPESAFE_API_KEY`. Add `motor.select` and the selector's HTTPS endpoint to the frozen allowlist. It is called only when an adapter produces multiple valid plans. Usage and cost appear in the receipt. Its byte-based token reservation includes configurable overhead and is an estimate, not a provider-enforced spending cap. An unprovable charge preserves the reservation. Tests use injected transports and do not send paid requests.
